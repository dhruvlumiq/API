...
const viewUser = async (req, res) => {
  ...
  try {
    ...
    if (!data) {
      res.status(404).json(createResponse(404, "Bad Id", "Id not found", false));
    } 
    else {
      data.birthday = dbDateFormatChange(data.birthday);
    ...
    res.status(400).json(createResponse(400, "Bad Request", error.message, null));
  }
};
...
const createUser = async (req, res) => {
  try {
    ...
    res.status(200).json(createResponse(200, "User created successfully", false, user));
...
const updateUser = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json(createResponse(404, "Invalid Id", "Invalid id found", null));
  }
  ...
  try {
    ...
    if (!data) {
      ...
      res.status(200).json(createResponse(200, "User updated successfully", false, data));
    }
  } catch (error) {
    res.status(400).json(createResponse(400, "Bad Request", error.message, null));
  }
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (!data) {
      res.status(404).json(createResponse(405, "Bad Id", "Id not found", false));
    } 
    else {
      data.birthday = dbDateFormatChange(data.birthday);
      res.status(200).json(createResponse(200, "User deleted successfully", false, data));
    }
  } catch (error) {
    res.status(400).json(createResponse(400, "Bad Request", error.message, null));
  }
